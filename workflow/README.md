...
- [`translation/`](translation/README.md)：**翻譯決策**（TranslationContext → Analysis → Candidate Space → Feasible → Selection → Finality）。Phase 1 contract／registry；**尚未**註冊 route、**尚未** runtime 投影。P0 regression：[`translation/examples/address-title-chen-xiaojie-id.yaml`](translation/examples/address-title-chen-xiaojie-id.yaml)。
