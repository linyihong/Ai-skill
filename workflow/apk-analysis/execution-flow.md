...
本文件定義授權 APK 分析的 agent 執行流程。分析方法見 [`analysis/apk/`](../../analysis/apk/)；工程智慧見 [`intelligence/engineering/analytical-reasoning/`](../../intelligence/engineering/analytical-reasoning/)。
...
**Unity runtime atlas restoration rule：** 要宣稱 2D 素材「完整還原」時，不能用 screenshot crop 或像素相似度當 source。先檢查 `UIImage.get_Sprite()` 的**宣告回傳型別**：若是 `UnityEngine.Sprite`，記錄 `Sprite.name`、texture、rect／textureRect、packed／rotation；若是 client 自訂 atlas handle（例如 `UIAtlasRect`），記錄 resource name、atlas 類型、entry id、normalized UV、PixelSize、scale／alpha 與 active texture 尺寸；**export gate**：`valid==true` 且 atlas Pointer／entry id 可讀，且 UV 不是 unit rect——未過 gate 不得 ReadPixels，改標 `screen-isolated`／`runtime-atlas-blocked`。`Texture2D` 可直接 encode；`RenderTexture` 必須在 Unity main／render-thread callback 內 bounded 執行 `RenderTexture.active` + `Texture2D.ReadPixels`，不可從 Frida worker thread 建 texture 後宣稱失敗。驗證至少要求 `UV × textureSize`、runtime PixelSize、匯出 PNG 尺寸三方一致；任一不一致即標 `runtime-atlas-unverified`。Choose-category／splash 中央 `preview_texture`（`SpriteName` 常空）視為合成預覽：畫面上看似光暈／主體／底座／字標分層，多半是藝術層而非可獨立 atlas 匯出的 UIImage；勿把 table logo、lobby tile、button chrome、in-cabinet BG 升格為 splash source。空 SpriteName 的 preview／title 還原優先序為 main-thread `get_Texture` dump（idle 需強制重繪）→ CDN 開啟窗 → GPU frame；可執行步驟留在 feedback lesson，不寫進 technique-free viewer HTML。畫面小圖示若不在 active `UIImage`／Sprite 清單，先檢查相鄰 `UIText` 的 Private Use Area codepoint，並用實際 font、size/style、texture size 與 `Font.GetCharacterInfo` glyph UV 驗證；這類結果標 `generated-glyph`，不可誤報成獨立 PNG，動態 font atlas UV 只算當次執行證據。Runtime body/style font 名稱不一定是 PUA icon 來源檔；若有候選 `.otf`/`.ttf`，要用 cmap 確認哪個檔真正含該 codepoint，再匯出 HTML 可用 PNG 或 `@font-face`。只有 sprite/handle/glyph identity/UV 不可取得、texture 不可讀或畫面是多 draw-call composite 時才升級 RenderDoc。Lessons：`feedback/history/apk-analysis/unity-il2cpp/2026-09-14_082500-unity-atlas-manifest-after-frida-sprite-name.md`、`feedback/history/apk-analysis/unity-il2cpp/2026-09-14_111500-unity-inline-icon-may-be-private-use-font-glyph.md`、`feedback/history/apk-analysis/unity-il2cpp/2026-09-14_113500-pua-icon-glyph-may-live-in-separate-icons-font.md`、`feedback/history/apk-analysis/unity-il2cpp/2026-09-14_134800-uiatlasrect-valid-false-null-pointer-blocks-uv-export.md`、`feedback/history/apk-analysis/unity-il2cpp/2026-09-14_134900-choose-category-preview-texture-is-composite-not-layer-sprites.md`、`feedback/history/apk-analysis/unity-il2cpp/2026-09-14_141200-restore-unity-preview-composite-via-main-thread-or-cdn.md`。
