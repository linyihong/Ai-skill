...
Plan source：[`plans/archived/2026-06-29-1430-preparatory-refactoring-workflow/_plan.md`](../../../plans/archived/2026-06-29-1430-preparatory-refactoring-workflow/_plan.md) Phase 1（completed / archived 2026-07-22）。
...
Review lens（dogfood / review 時逐項檢查）：

| failure | 徵象 | recovery |
|---------|------|----------|
| intent oscillation | `structure ↔ feature` 反覆；`feature → structure` 無 `explicit_reopen_reason` | 停止；補 reason、改 plan，或回 intake 重判雙軸 |
| structure inflation | 連續 structure steps 無 `exit_when` 收斂；永遠「再抽一層」 | 觸發 `force_exit_when`；縮 scope、`direct_change`，或回 intake |
| fake equivalence | checkpoint 標了 pass / 有 evidence 欄位，但 regression 未跑或 observable 實際已變 | 降級 claim；補 parity / regression；必要時重開 structure |
| abstraction orphan | 新 seam / abstraction 做出來，下一步 feature step 未引用 | 觸發 `abstraction_not_used_by_next_feature`；刪 abstraction 或改 feature step 踩上去 |
| compatibility collapse | 舊 plan 被誤要求補 `intent` / `execution_mode`；或缺欄位被當成非法 | 確認 `default_execution_mode: direct_change`、`missing_intent_on_steps: allowed`；僅 preparatory mode 強制 intent |
| infinite refactor | 同 structure inflation；歷史別名 | 同上 |
