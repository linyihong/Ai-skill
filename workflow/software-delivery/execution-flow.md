...
> **Domain Model**：本檔描述 **Process** 層（何時載入哪個 slice）。Asset 分類與 Policy 規則見 [`README.md`](README.md) §Framework Domain Model；完整驗證見 [`phase-0-classification-matrix.md`](../../plans/archived/2026-07-16-0945-software-delivery-framework-domain-model/evidence/phase-0-classification-matrix.md)。Process **不**定義 placement / owner。
...
> | UI contracts（Screen Mapping / Consumer / Screen / ViewModel） | **已抽出** → [`ui-contracts.md`](ui-contracts.md)（含 Screen Mapping、Consumer Contract、UI Behavior Contract、Screen Contract、Frontend ViewModel Contract、Accessibility Contract、Screen Traceability、**Pattern Knowledge Lock**） | `sd-ui-contracts` | 前端、行動、CLI、SDK 或其他 consumer surface 需要平行實作或 AI 生成 UI / state / tests；overlay／pattern 選型不明或宣告 pattern 對齊時同載 Lock |
...
> | Implementation（執行核心） | [`implementation/README.md`](implementation/README.md)（含 [`execution-modes.md`](implementation/execution-modes.md)、[`multi-repository-workspace-mode.md`](implementation/multi-repository-workspace-mode.md)） | `sd-implementation` | 實際進行程式碼變更 |
> | Delegated execution（委派執行 loop） | **已抽出** → [`delegated-execution.md`](delegated-execution.md)（角色×證據責任矩陣、verification backfill、deliverables、slice 關閉狀態 C1–C5、Verifier V1–V4；loop 契約 canonical 在 [`plans/README.md`](../../plans/README.md) §Delegation） | `sd-delegated-execution`（maturity: candidate） | 交付任務已宣告 `delegation.enabled: true` 進入執行；orchestrator 拆 slice / 寫 brief / 判定合規關閉 |
