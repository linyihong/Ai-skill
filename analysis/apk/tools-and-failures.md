...
| Choose-category／splash `preview_texture` composite | 中央 marketing art 常掛在 `preview_texture`（`SpriteName` 空）；畫面分層 ≠ 多張可匯出 UIImage。 | 標 `verified-composite-identity`／`visual-layer-in-composite`；否決 table logo、lobby tile、button chrome、in-cabinet BG。字標節點（如 `game_title`）需獨立 texture 證明。Lesson：[`choose-category-preview-texture-is-composite-not-layer-sprites`](../../feedback/history/apk-analysis/unity-il2cpp/2026-09-14_134900-choose-category-preview-texture-is-composite-not-layer-sprites.md)。 |
