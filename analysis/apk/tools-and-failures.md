...
| Frida | Java/native/Dart hook，抓 request object、decode function、connect trace。 | 優先 hook 高語意點；注意 crash、ANR、輸出量。Android：`attach(package)` 失敗時改 `adb pidof` + `enumerate_processes()` / **PID**（launcher 標題 ≠ applicationId）。CLI 17 的 `-q` 必須搭配 `-t`，否則 load 完就退出。 |
...
| Custom runtime atlas（`UIAtlasRect` / equivalent handle） | Wrapper 不回傳 `UnityEngine.Sprite` 時，以 atlas entry + UV 從 active texture 匯出完整 RGBA。 | 先檢查 `get_Sprite()` 宣告回傳型別；**export gate**：`valid==true` 且 atlas Pointer／entry id 可讀，且 UV 不是 unit rect；未過 gate 標 `screen-isolated`／`runtime-atlas-blocked`，不可 ReadPixels。`RenderTexture` 需在 Unity main/render thread 以 `ReadPixels` 匯出。驗證 `UV × textureSize == handle PixelSize == PNG size`。Lesson：[`uiatlasrect-valid-false-null-pointer-blocks-uv-export`](../../feedback/history/apk-analysis/unity-il2cpp/2026-09-14_134800-uiatlasrect-valid-false-null-pointer-blocks-uv-export.md)。 |
| Choose-category／splash `preview_texture` composite | 中央 marketing art 常掛在 `preview_texture`（`SpriteName` 空）；畫面分層 ≠ 多張可匯出 UIImage。 | 標 `verified-composite-identity`／`visual-layer-in-composite`；否決 table logo、lobby tile、button chrome、in-cabinet BG。還原優先序：main-thread `get_Texture` dump（需 UI 重繪）→ CDN 開啟窗 → GPU frame。Lesson：[`choose-category-preview-texture-is-composite-not-layer-sprites`](../../feedback/history/apk-analysis/unity-il2cpp/2026-09-14_134900-choose-category-preview-texture-is-composite-not-layer-sprites.md)、[`restore-unity-preview-composite-via-main-thread-or-cdn`](../../feedback/history/apk-analysis/unity-il2cpp/2026-09-14_141200-restore-unity-preview-composite-via-main-thread-or-cdn.md)。 |
| 同一 cabinet 三視覺平面 + 進桌動畫棧 | 大廳 tile、splash preview、進桌 cabinet 是不同成品；進桌符號靠協議 ID→Spine／Animator／Particle，不是 5×3 靜態拼圖。 | 先標 plane；進桌 checklist 含 hierarchy、底板 Sprite、Spine、Animator、Particle、ID map、shared HUD 邊界。Lesson：[`slot-three-visual-planes-and-in-cabinet-anim-stack`](../../feedback/history/apk-analysis/unity-il2cpp/2026-09-21_091600-slot-three-visual-planes-and-in-cabinet-anim-stack.md)。 |
...
| Handle 有 PixelSize／Name，但 runtime atlas 匯出 empty 或整頁 | `valid=false`、Pointer null、UV unit rect 的半載入狀態；或誤把 Icons-font 同義 glyph／imgcache MSE 當 source。 | Export gate 未過就停；標 `screen-isolated`；字形要做 cmap／glyph 比對。見 `uiatlasrect-valid-false-…` lesson。 |
| Splash 中央插畫拆成多個 O-id 卻全是假陽性 PNG | `preview_texture` 是合成預覽；視覺層沒有獨立 SpriteName。 | Plane 分類 + rejectedFalsePositives；screen band 只做 visibility。見 `choose-category-preview-texture-…` lesson。 |
| 用 lobby／splash 圖「還原」進桌 reel，或把 layer-stack 色塊當可匯出分層 PNG | 三平面混用；進桌是 ID→prefab 動畫棧。 | 分 lobby／splash／in-cabinet；進桌列 Spine／Animator／Particle。見 `slot-three-visual-planes-…` lesson。 |
...
frida -D <device-serial> -f <package-name> -l hook.js -q -t 90
```
