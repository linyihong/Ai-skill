...
**Protocol Core**（修改 = 重新評估成熟度）vs **non-core**（Validated Instances、RI 索引、editorial、optional patterns 如 [§Layer Growth Rhythm](architecture-evolution-protocol.md#appendix--layer-growth-rhythmoptional-governance-pattern)）。詳見文件 §Protocol Core · §Protocol Changelog。
