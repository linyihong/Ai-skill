...
| 8 | Plan Evidence Convention (commits 30e794c + 3e33919, mechanical) — **first post-promotion sample**（經 ECS candidate `C-12cd0d07` accept，criteria_source: human） | delegation-loop dogfood 證據堆積 + `L449` 行號引用漂移（829ba60） | `governance/lifecycle/plan-evidence.md`（policy：README 結構 / 引用規則） | `runtime/core-bootstrap.yaml` §per_commit_obligations `obligation.commit.plan_evidence_convention` | core-bootstrap contract → `runtime.db` generated_surfaces（index refresh 3e33919） | `validatePlanEvidenceConvention`（commit-msg block）+ `warnPlanEvidenceLineNumberCitations`（warning）in `plan_evidence.go` | `plan_evidence_test.go`（6 Go tests）*(scenario-yaml sub-form pending)* |

**Sample #8 note — template validation evidence（≠ ECS maturity evidence，分開累積）**：#8 是 template
**promote 之後**第一個依既有 template 收進的新 sample——它驗證的不是 sample 而是 **template 自身的可用性**
（`Template promoted → new subsystem appears → template successfully classifies it`），開始具備解釋新案例
的能力而不只整理歷史案例。此證據**不推動 ECS phase2_gate**。
**Reviewer note（非 criterion、非 exception）**：`enforcement-registry.yaml` 無對應 entry，目前由
`governance/lifecycle/` 作為 rule authority；此差異不影響 template applicability，保留作後續觀察。
