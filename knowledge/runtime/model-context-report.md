...
| `route.models.model-aware-routing` | `models/README.md` | `source-backed` | 修改 model strategy 會影響 runtime routing，需讀 profiles、compression 與 routing source。 |
| `route.memory.retrieval-activation` | `memory/README.md` | `source-backed` | Memory replay 會影響 context budget、evidence scope 與 contamination risk，需讀 source-backed governance。 |
...
| `route.runtime.capability-context` | `runtime/capability-context.yaml` | `summary-first` | Invoke validation is CLI-driven; contract YAML + registry are primary reads. |
| `route.governance.cognitive-stance` | `governance/cognitive-stance.md` | `summary-first` | Stance contract is compact; expand ADR-014 only when extending enum. |
...
| `route.ai-tools.agent-onboarding` | `ai-tools/agent-onboarding.yaml` | `source-backed` | 只有在新增或修改 agent 工具設定、adapter 或 bootstrap 入口時才載入。 |
| `route.ai-tools.new-project-onboarding` | `ai-tools/new-project-onboarding.yaml` | `source-backed` | 只有在修改新專案初始化、工具清單或 project bootstrap files 時才載入。 |
...
| `route.runtime.cognitive-modes` | `runtime/cognitive-modes.yaml` | `summary-first` | Cognitive modes contract 本身是 YAML；summary-first 足夠解析 mode；只有需要 subsystem integration 細節時才展開 integration YAMLs。 |
...
| `route.governance.system-upgrade` | `governance/lifecycle/system-upgrade-governance.yaml` | `source-backed` | Framework 升級或 runtime path 變更會影響執行語意，需要直接讀 executable contract 而非只讀 routing entry。 |
...
| `route.knowledge.glossary` | `knowledge/glossary/ai-skill.md` | `source-backed` | Glossary 在 plan、BDD discovery、framework discovery、pre-build interrogation 遇到詞義衝突或需要 canonical 定義時才載入；不佔用 bootstrap 預算。  |
...
| `route.workflow.legal` | `workflow/legal/execution-flow.md` | `source-backed` | 法律任務要求 source-backed 引用（法規版本 + 查核日）與可解釋的策略推理； summary-first 會讓未查證主張以確定語氣輸出，屬本 domain 最高風險失效模式。  |
| `route.workflow.investment` | `workflow/investment/execution-flow.md` | `source-backed` | 投資建議失敗模式是無舉證喊單與假精確機率；必須 SOURCE_BACKED evidence 與 STRICT intake／authority／uncertainty／DVA gates。  |
