...
> **專案證據邊界**：inner commit、class 名、DB job id、merge SHA 留于 `<PROJECT_ROOT>` sub-plan 05 §执行纪录与 `preflight-feedback-log.md`；Ai-skill 只保留 generalized dogfood metrics 与契约回饋。
...
- **Transport**：Cursor orchestrator；纠偏阶段有 Executor + Verifier loop；**关闭后**多次 `SURGICAL_BYPASS` 直改 manageCode + orchestrator merge/push **无新 Verifier**。
- **Repo**：`<PROJECT_ROOT>` 外层 + `<INNER_REPO>/manageCode`（`feature/push-ninej2-sync` → `master` @ consumer merge）。
...
| 2j 缺口 | 2k 纠偏后表现 |
|---|---|
| 0 Verifier | **R1 Verifier** + S6 outer L1–L3 + `test:push-sync` green |
...
| # | 现象 | 根因分类 | 应有证据层 |
|---|---|---|---|
| F1 | 消息模版下拉为空 | **runtime stale JVM** — `approved-options` 新码未 reload；重启前 API 对 `{}` 返回 0 条 | V5：**restart-aware** + 测 API 契约（非仅 list） |
...
| F3 | 远程同步「任务进行中」 | **拓扑**：`ninej2_sync_jobs` pending 无 Worker 消费；非「未部署 Admin」 | V5-W：**sync-remote 202 + worker 消费**；dev README 须写 worker |
| F4 | Worker 执行 pull 失败 | **映射**：9j2 行缺 `app_url` → PG NOT NULL | V5：**pull upsert 样本行**（非 mock-only） |
...
| F6 | orchestrator merge/push master | **release leg** 在 loop 外（延续 2d′） | 与 implementation 验收分离；merge 前可选 smoke |
...
| F2 商户解析 | **fix**（code） | acceptance-violation — 手验路径未覆盖 | `resolveCreateMerchantId` + integration test |
| F3 pending job | **fix**（ops） | 环境拓扑非单点 API | 启动 worker；可选 pending TTL/失败可重入队 |
| F4 app_url null | **fix**（code） | pull 映射缺口 | AdminPushRemoteSyncService 字段 fallback |
...
| F7 补测 | **fix**（test） | 闭合 2j F4 债务 | 纳入 `test:push-sync` |
...
1. **`v5-ui-critical-path`** — combined / user-visible slice 的 V5 须覆盖 **client-admin 关键表单所调 API 链**（从 `hook` + `src/api` 提取），不能仅 list/sync-remote 注册性 smoke。
2. **`sync-remote-requires-worker`** — `POST …/sync-remote` 返回 202 只证明入队；**验收须 Worker 消费**或 integration 测 job 状态迁移；dev 文档须 **Admin + Worker 双进程**。
3. **`pending-job-stale-guard`** — `findActiveJobId` 对永久 pending 阻塞重试；consumer 候选：**TTL 标 failed** 或 orchestrator 运维脚本（doc-only 先记）。
