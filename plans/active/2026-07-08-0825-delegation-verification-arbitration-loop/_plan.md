...
| Q5 | Schema promotion 門檻：什麼證據才允許動 delegation schema（如 `delegation.verification`）？ | Phase 3 | open | 門檻明文化；未達門檻則明確記錄維持 doc-only | kit §2c + **§2d** 增強信號；consumer 機械 gate（2c/2d）後 orchestrator 零 manageCode diff；**§2g 第二個獨立 consumer**（ExternalRepoA overlay + backfill + 自建 gate，BDD 7/7）——consumer-layer 自理模式 ×2 成立，slice 各項 promotion eligibility 達標、評估歸 Phase 3 批次 — schema 本身**仍維持 doc-only** |
