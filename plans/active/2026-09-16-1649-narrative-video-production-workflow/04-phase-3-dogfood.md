...
| `font_shrink_past_min` | 為了 fit 縮到 min 以下，或短句放大超過 max | **contract**：mechanical `[min,max]`＋`step`；到 min 仍 fail → 重切 unit。見 [`evidence/2026-09-24-font-size-hard-bounds.md`](evidence/2026-09-24-font-size-hard-bounds.md) |
...
| `max_lines_as_target` | `max_lines: 2` 被做成「盡量兩行」 | **contract** 已補：上限＋minimize_lines；glyph 可行集＋selection.policy；記 `layout.lines`。見 [`evidence/2026-09-22-max-lines-is-bound.md`](evidence/2026-09-22-max-lines-is-bound.md) |
| `layout_breaks_semantic_span` | 為 fit 截斷詞／專名，或同 cue 上下行各自縮放 | **contract** 已補：wrap lossless＋protected spans＋cue-uniform typography；無解回 Speech Unit。見 [`evidence/2026-09-25-semantic-safe-wrap-uniform-typography.md`](evidence/2026-09-25-semantic-safe-wrap-uniform-typography.md) |
| `layout_unscored_or_equal_split` | 字數均分換行，或 layout 自創未評分斷點 | **contract** 已補：Caption Composition；只從 scored `semantic_break_candidates` 選 wrap。見 [`evidence/2026-09-25-caption-composition-semantic-breaks.md`](evidence/2026-09-25-caption-composition-semantic-breaks.md) |
