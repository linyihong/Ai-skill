...
```text
observable → text resolution → evidence_unit → narrative window
  → event assembly + candidate relations → story event
  → typed story-state claim（若有）→ narrative decision (script / EDR)
```

聚合層見 [`16-evidence-unit.md`](16-evidence-unit.md)：**先停擴 observable**。`evidence_unit` 不是劇情，只是同一時間窗的證據包。

中間必過 **Relevance Assessment**：不問「這一句是不是劇情」（單句會誤殺伏筆），而在 narrative window 中問 evidence 組合形成何種 event／relation，以及是否支持狀態 claim。`narrative_role` 允許多類與 `unknown`，禁止每句二元分類。多句 dialogue／action／OCR／face 聚成 **event candidate**，不要四句四個劇情點。Local relevance ≠ Narrative relevance。
...
[`17-story-promotion-gate.md`](17-story-promotion-gate.md)。同一 run 的人工複核
進一步證明 narrative consumer 沒吃到 resolved text，且 `dialogue_cluster`
不是充分的 event representation；見
