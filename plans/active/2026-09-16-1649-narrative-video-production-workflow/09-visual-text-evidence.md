...
`role.candidate` + `resolver: mechanical` + `final: null` 是正確形狀：機械「像字幕」≠ 系統宣告就是字幕。空間／時間清楚時不必 LLM vision。掃區與 fallback：[`13-mechanical-visual-text-probe.md`](13-mechanical-visual-text-probe.md)。右上角高覆蓋率的「林雪」不得因像人名就進 ASR 人名仲裁。仲裁見 [`12-evidence-refinement.md`](12-evidence-refinement.md)。下一層聚合：[`16-evidence-unit.md`](16-evidence-unit.md)。Role ≠ deletion：[`24-ocr-role-projection.md`](24-ocr-role-projection.md)。Phase 3 **不**改 workflow。
