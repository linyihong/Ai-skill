...
Face／voice **不是**唯一 identity key（雙角色、變裝、回憶、配音、遮臉、音質差）。它們只當 evidence，避免早期錯誤污染 series_cast。
...
第一集 unnamed entity 比例；後來命名是否用 link 而非覆寫；face／voice 衝突時有沒有當 identity key。沒有普遍需求就不進 workflow。
