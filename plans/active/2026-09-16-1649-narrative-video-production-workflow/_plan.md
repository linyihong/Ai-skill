...
7b. Locale packs         content／timing／layout；generated 口播 cue 時軸 = speech artifact
...
- [x] Speech Unit／timing authority：generated 口播 cue 時軸 = speech artifact；TTS 仍 adapter；未加 7c TTS
- [x] Subtitle `max_lines` 是上限＋`minimize_lines`；layout.lines 與 max_lines 分記
- [x] Font min／max／step 機械硬閘；到 min 仍 fail → 重切 Speech Unit
- [x] Wrap lossless＋protected spans；同 cue typography 統一；無合法 break 回 Speech Unit
- [ ] Subtitle layout engine：glyph＋constraints＋profile；**max_lines 上限已進 workflow**；AI 不寫 profile；候選 invariant 14 未凍結十條
