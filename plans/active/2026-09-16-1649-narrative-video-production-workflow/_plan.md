...
- [ ] 卡住不自動加欄位／加 phase（dialogue optional 維持；action／visual／series_cast **不**寫進本 phase workflow）
- [ ] 虛構 YAML 示範不算本 phase
