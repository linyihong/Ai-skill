...
機械與語意走不同路徑：watermark 區域反覆出現 → pattern candidate → validation → mechanical rule；稱呼／關係 → semantic candidate，不得一次 resolved。字幕 sanitization／vocative 槽位同樣先 inbox，見 [`22-phonetic-text-reconstruction.md`](22-phonetic-text-reconstruction.md)、[`23-sanitization-anomaly-audit.md`](23-sanitization-anomaly-audit.md)。Typography preferred 的跨集調整同樣先 inbox，見 [`28-layout-review-loop.md`](28-layout-review-loop.md)。
