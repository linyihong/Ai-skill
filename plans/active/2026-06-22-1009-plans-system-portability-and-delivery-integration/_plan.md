...
| Q4 | plan-first 與 pre-build-interrogation / Preflight 分工（plan 是 artifact、preflight 回改 plan，loop 非線性）？ | 02 | open（criterion 1/2 met） | intake loop 段落落地 ✅ + 一次真實 intake 含 preflight 回改實例 ⏳ | `workflow/software-delivery/intake.md` §Plan-First Ordering（loop 模型 + 三欄分工表 + 規模分級豁免，2026-07-01）落地；分工已界定（interrogation 餵入 / plan artifact / preflight 回饋）。**未 close**：待一次真實 software-delivery intake 走過 loop 並留下可驗證的 **planning artifact 演化**（`T0` draft → `T1` 獨立 preflight feedback → `T2` revised planning artifact，非限定 `plan.md`）後由 02 owner 標 resolved。候選評估見 02 §完成條件（strong-supporting 一例、backfill 一例已排除） |
