...
| Q4 | plan-first 與 pre-build-interrogation / Preflight 分工（plan 是 artifact、preflight 回改 plan，loop 非線性）？ | 02 | open（**ordering ✅ verified / feedback-loop ◑ partial**） | 分工界定 ✅ + intake loop 落地 ✅ + **A. Ordering Evidence ✅（Brower）** + **B. Feedback Loop Evidence ◑ partial** | 拆兩子命題（2026-07-06）：**A. Ordering**（plan-first 是否真發生）已 **Verified**——Brower `docs/plans/active/2026-07-03-verification-code-center.md` 真實、`T0`<impl、Git 時序可驗、非 backfill。**B. Feedback Loop**（preflight 是否**實質**改 plan）仍 **open**——缺一個 `preflight→issue→plan modified→reason preserved` 且 modification ∈ {scope/sequencing/dependency/acceptance}（非 link/ref/typo）的乾淨案例（Brower doc-split 僅改 3 行 reference，未達 materiality）。**Q4 close 只 gated on B**；不 hunt、不降門檻。詳見 02 §完成條件 |
