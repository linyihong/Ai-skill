...
| Q4 | plan-first 與 pre-build-interrogation / Preflight 分工（plan 是 artifact、preflight 回改 plan，loop 非線性）？ | 02 | open（criterion 1/2 met） | intake loop 段落落地 ✅ + 一次真實 intake 含 preflight 回改實例 ⏳ | `workflow/software-delivery/intake.md` §Plan-First Ordering（loop 模型 + 三欄分工表 + 規模分級豁免，2026-07-01）落地；分工已界定（interrogation 餵入 / plan artifact / preflight 回饋）。**未 close**：待下一個真實 software-delivery intake 走過 loop（含一次 preflight 回改 plan）後由 02 owner 標 resolved |
