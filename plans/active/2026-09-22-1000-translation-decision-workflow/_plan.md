...
**Status**: in-progress — Phase 0–3 + I12–I13 + **I14–I16 semantic／syntax**（[`11`](11-semantic-syntactic-realization.md)）。Phase 4 optional。證據 [`evidence/`](evidence/README.md)。
...
- [x] SoT 可獨立閱讀；I1–I16 寫進 contracts（content.type／failure_pattern／semantic／syntax）
- [x] ≥3 expression examples + P0 陈小姐 **id + ja** + title + **老張** social-address fixtures
- [x] README 說清 Context／Analysis／Semantic／Realization／Guards／Selection／Target Realization／Finality／Failure Learning
- [x] subtitle adapter 餵 NVP content_gate，不碰 timing／layout
- [x] Phase 3 dogfood evidence 入庫（含 title invented_information）
- [x] 未註冊 route、未 runtime 投影、未另開獨立 semantic-workflow、未要求 source word order == target
