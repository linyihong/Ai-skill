...
當主計畫執行中需要拆出有獨立 acceptance、跨多 phase、需獨立 sign-off 或可獨立 archive 的支線時，用 **plan tree** 表達 main ↔ sub 階層，而不是把 Phase 6/7/8 塞爆主計畫或另開孤立 plan 用 prose 連結。完整治理規則見 [`governance/lifecycle/plan-tree-hierarchy.md`](../governance/lifecycle/plan-tree-hierarchy.md)；落地範例見 [`archived/2026-06-02-1200-plan-tree-hierarchy-governance/_plan.md`](archived/2026-06-02-1200-plan-tree-hierarchy-governance/_plan.md)。
...
| `validatePlanEvidenceConvention` | block | `evidence/` 使用時：主計畫必須在 `_plan.md`（禁止頂層 `<slug>.md` 並行）；README + Run 索引同步（見 [`governance/lifecycle/plan-evidence.md`](../governance/lifecycle/plan-evidence.md)） |
...
## Plan Evidence Accumulation（證據子目錄）

多 session 量測、dogfood、delegation loop 的**證據全文**放各 plan folder 的 **`evidence/`** 子目錄；`_plan.md` 與 companion 檔只留摘要 + **檔案路徑連結**（**禁止** `L449` 行號引用，避免編輯漂移）。完整規則見 [`governance/lifecycle/plan-evidence.md`](../governance/lifecycle/plan-evidence.md)；模板見 [`templates/plan-evidence/README.md`](templates/plan-evidence/README.md)。

| 元件 | 說明 |
|---|---|
| `evidence/README.md` | 必填：引用規則 + Run 索引表 |
| `evidence/<run-id>-<slug>.md` | 各 run 全文（量測欄、契約回饋） |
| `validatePlanEvidenceConvention` | commit-msg **block**：staging `evidence/**`（或頂層 `<slug>.md` 且 `evidence/` 已存在）時：必須有 `_plan.md`、禁止 flat sibling、README 存在且索引覆蓋所有 `.md` |
| `warnPlanEvidenceLineNumberCitations` | commit-msg **warning**（不 block）：plan folder 內 `L123` 行號引用 |

範例：[`active/2026-07-08-0825-delegation-verification-arbitration-loop/evidence/`](active/2026-07-08-0825-delegation-verification-arbitration-loop/evidence/README.md)

**扁平多檔 → 資料夾（情境 A）**：若**同一 plan** 在 `plans/active/` 頂層出現 `<slug>.md` 與 `<slug>-<companion>.md`（例如 dogfood evidence），應收斂為 `<slug>/_plan.md` + `NN-<companion>.md`（companion 不必是 sub-plan）。`validatePlanTreeFolderConvention` 對 staged cluster 發 warning；機械遷移：`ai-skill plans folderize --cluster <slug> --dry-run` / `--apply`。範例：[`archived/2026-06-29-1430-preparatory-refactoring-workflow/`](archived/2026-06-29-1430-preparatory-refactoring-workflow/_plan.md)。
...
**多 slice / 一口氣執行（dogfood 2p，2026-07-13）**：

| 規則 | 說明 |
|---|---|
| **多 todo ≠ 單 Task 包辦** | 使用者說「一口氣做完 / Don't stop until all todos」時，語意是 **逐 slice 重複** `brief → Executor → Verifier → 仲裁`；**禁止**合併成一個 Task 同時 implement + verify（對照 2j 負向）。 |
...
| 檔案 | 狀態 | 說明 |
|------|------|------|
| [`active/2026-09-16-1649-narrative-video-production-workflow/_plan.md`](active/2026-09-16-1649-narrative-video-production-workflow/_plan.md) | in-progress（Phase 3 dogfood） | Phase 1–2 PASS。真實片子驗證契約。`semantic_context` optional；Series Cast 僅 observation、不改 workflow。 |
| [`active/2026-09-22-1000-translation-decision-workflow/_plan.md`](active/2026-09-22-1000-translation-decision-workflow/_plan.md) | draft | Translation Decision Workflow：TranslationContext（P0）+ Locale Resolution → TDR；**陈小姐→id-ID** dogfood／locale_consistency regression（[`04`](active/2026-09-22-1000-translation-decision-workflow/04-dogfood-case-address-title-id-ID.md)）。Phase 1 doc-only contracts。 |
| [`archived/2026-07-16-0945-software-delivery-framework-domain-model/_plan.md`](archived/2026-07-16-0945-software-delivery-framework-domain-model/_plan.md) | ✅ completed | Software Delivery Framework Domain Model：Asset／Policy／Process 與最小文件面完成；兩個 consumer 六例覆核。選做通用 Automation 實作 deferred（owner／重啟條件見 plan §3b），不升 ADR。 |
| [`active/2026-08-31-1032-3d-character-production-workflow/_plan.md`](active/2026-08-31-1032-3d-character-production-workflow/_plan.md) | in-progress（Phase 3 dogfood 十一條 PASS；live = prototype） | Phase 4／5／6 blocked。Detector no_match EXPECTED。 |
| [`archived/2026-07-30-0950-knowledge-governance-runtime/_plan.md`](archived/2026-07-30-0950-knowledge-governance-runtime/_plan.md) | ✅ completed | Knowledge Governance Engine (KGE)：portable `scripts/ai-skill-cli/portable/kge/`（29 rules）+ Ai-skill adapters（commit-msg count / pre-push `kge check` / CLI validate·diagnose / CI `--advisory`）。Phase 0–2 Core Exit + D9 multi-adapter；glossary 三詞；Delegation Loop 交叉引用。**Later deferred**：Plans pack、IDE host、external dogfood、Unified Execution explore、ADR remainder。Final verify [`evidence/final-core-exit-verification.md`](archived/2026-07-30-0950-knowledge-governance-runtime/evidence/final-core-exit-verification.md)。 |
| [`active/2026-07-10-0955-final-form-feedback-execution-closure.md`](active/2026-07-10-0955-final-form-feedback-execution-closure.md) | in-progress | Final Form — Feedback Execution Closure & Model-Neutral Stability：系統最終形態 roadmap。北極星三性質：(1) feedback 靜默流失率=0（deferred-feedback ledger `feedback/pipeline/deferred/` + resurface + gated 機械化）；(2) model-neutral cognition parity；(3) mechanical-first stability。含 §全系統架構審計 P1–P12 + Last-Fable 提示詞對照。**進度**：Phase 1（deferred ledger，併發 session 落地）✓；**Phase 5b（模型調度 + 判斷力外化）✓ 完成**——F1 weak-model-rule-authoring / F3 judgment-rubrics / F6 institutional-degradation-modes（governance/）、F2 model-tier-escalation（models/routing/）、F5 edit-authority-map（enforcement/）、F4 delegation-prompts（templates/）全數入庫並接入索引，經 fresh Haiku verifier 對抗審查 8 findings 逐條修正（F2 其他工具 model 參數 pending Q11）。Phase 2–4/5/6–10 pending。First landing doc-only；機械化 gate entry-condition gated。 |
| [`archived/2026-07-14-0856-ui-pattern-knowledge-workflow/_plan.md`](archived/2026-07-14-0856-ui-pattern-knowledge-workflow/_plan.md) | ✅ completed | UI Pattern Knowledge：RC1/RC2 已 closed；2026-09-09 D9 = Continue，計畫歸檔，doc-only 維護交由 maintenance-governance；Optional L2／ADR／glossary promotion 未啟動。 |
| [`active/2026-07-30-2101-legal-workflow-domain.md`](active/2026-07-30-2101-legal-workflow-domain.md) | in-progress（implementation closed） | Legal Workflow Domain：[`workflow/legal/`](../workflow/legal/README.md) — domain 邊界是**法律任務**而非合約文件，第一級分派維度 legal task type（draft/review/explain/compare/research/due-diligence/strategy/negotiation-support/lifecycle）。流程為「問問題 → 推理最佳策略 → 使用者決策 → 才決定怎麼寫」：[`strategy/`](../workflow/legal/strategy/README.md) 是共同 stage（Pass 1 provisional → Research → Pass 2 confirmed），強制 Decision Reasoning 四欄。Jurisdiction P0、法規引用強制版本+查核日、背調三層→條款影響、Green/Yellow/Red risk tier（Red 短路 Escalation Card，**不出實質建議**）。同批抽出 [`cross-cutting/decision-support/`](../workflow/cross-cutting/decision-support/README.md)（legal = converged case **1／3**）。Runtime：`route.workflow.legal` + `user_keyword_legal_work` signal，detector 實跑命中且 counter-case 不誤觸 software-delivery；7 scenarios。**Phase 1–7 完成但不 archive**（4 項 stakeholder ⏳ + Q1/Q4–Q11 未結 + ADR criteria 需 3 個真實任務）；適用 §不搬移的例外情況第 1 條。 |
| [`active/2026-07-08-0825-delegation-verification-arbitration-loop/_plan.md`](active/2026-07-08-0825-delegation-verification-arbitration-loop/_plan.md) | in-progress | Delegation Verification & Arbitration Loop：三角色委派閉環（orchestrator 規劃/切分/仲裁不執行、executor 憑 brief、verifier fresh-context 只產證據）。**Execute 意圖 = mandatory loop**（2j 裁決）；dogfood 2a–2s（含跨域 **2e Research** + **2s Architecture**；Knowledge 仍缺 → stage 2 = 2/3）→ 證據在 `evidence/`。研究線：ERA（Evidence constrains Decision，v2 凍結為工作模型、v3 候選）+ Phase 3 三分類決策框架，批次裁決 deadline 2026-08-31。**2026-07-21**：External Validation（Graph Engineering）確認 Loop-first／Governance-first；roadmap 重排（非研究方向修正）— Q5 重述為 **Shared State Contract Promotion**；優先序 Shared State → Transition/Edge → Join/Fan-in → Graph Visualization；Auto Orchestrator / Generic Graph Runtime = out of current research scope。doc-only、不建自動 orchestrator。 |
| [`archived/2026-06-29-1430-preparatory-refactoring-workflow/_plan.md`](archived/2026-06-29-1430-preparatory-refactoring-workflow/_plan.md) | ✅ completed (doc-only) | Implementation Execution Mode — Structure Preparation：於 `sd-implementation` 內加 `direct_change` vs `preparatory_refactoring` execution mode（**非新 lifecycle stage / slice**）+ Change Intent Lock（structure/feature intent、intent state machine、illegal transition）+ Observable Equivalence Checkpoint + stop condition（exit_when / force_exit_when / avoid）+ `change_kind` × `execution_mode` 雙軸 intake。正文 [`implementation/execution-modes.md`](../workflow/software-delivery/implementation/execution-modes.md)；Phase 3 四檔 cross-link + glossary 4 詞（`implementation_execution_mode` 等，避 cognitive `execution_mode` collision）。Dogfood triple path：`01` force_exit **verified** + `03` happy-path **verified**（Gate A observable equivalence proven）+ `02` **partial-verified**。**enforcement/validator hook 與 Promoted（SHA push + 獨立稽核）= 明確 follow-up**，非本 doc-only 計畫 scope。 |
| [`archived/2026-07-06-review-architecture-adr/_plan.md`](archived/2026-07-06-review-architecture-adr/_plan.md) | ✅ completed (auto-detected) | Review Architecture ADR-013：Phase 0a–0d + 1.1–1.4 + 2.1–2.4 全部完成。**ADR-013 Accepted → Completed**（reject D1 `cognitive_role` primitive；accept D2 `context.stance` on capability invoke）。Review = cross-cutting capability invoke（`fault_finding | default`），非 workflow phase / slice。落地 runtime routing、doc drift lock、canonical ownership lock、4-case contract regression。**不開 Phase 3**；stance taxonomy growth → **ADR-014**（Proposed）。 |
...
| [`archived/2026-05-28-1830-plan-archival-audit-validator.md`](archived/2026-05-28-1830-plan-archival-audit-validator.md) | draft | Plan Archival Audit Validator — Gen3 audit plan §Phase 7 follow-up：將「archive 時所有 `- [ ]` 必須翻 `[x]` 或在 body 明文交代」這條 manual 規則機械化成第 19 個 commit-msg validator `validatePlanArchivalAudit`（block default；opt-out `[skip-plan-archival-audit]`）。3 scenarios + ≥ 5 fixture tests + dogfood archive。**Sequencing：可獨立排程；建議在 Gen3 audit plan 進 archived 之前 graduate 以保護該 archive commit**。 |
| [`archived/2026-06-06-1800-sanitization-mechanical-enforcement.md`](archived/2026-06-06-1800-sanitization-mechanical-enforcement.md) | ✅ completed | Sanitization Mechanical Enforcement：metadata-derived forbidden tokens（`derived_match_tokens`）+ shared-layer topology + pre-commit staged-content scanner，`rule_classes[sanitization]` 已 **coverage=mechanical**（無 opt-out）。Phase 0–2 / 1A–1D / Phase 4 完成；衍生 Failure Authority invariant + Authority Classifier。剩 Phase 3（LLM review，deferred）+ 30 天 runtime_observed 觀察窗。 |
| [`archived/2026-06-16-1030-interaction-hazard-review-workflow.md`](archived/2026-06-16-1030-interaction-hazard-review-workflow.md) | draft | State Trust Transition **promotion discipline**：primitive→evidence→consumer；四栏 Ownership Map（State \| Owner \| Invalidation Event \| **Recovery Boundary**）；验 trust transition 闭环；roadmap A0→A1→B→**B.5 rename pressure**→C→D；ADR 含 predictive interception + template field survival；**do not optimize for becoming a primitive**。 |
| [`active/2026-06-16-1131-evidence-candidate-system.md`](active/2026-06-16-1131-evidence-candidate-system.md) | in-progress | Evidence Candidate System（跨 plan 候選證據層）：**Phase 1 完成（observation infra），進 observation period，Phase 2 gated 不主動建**。解「人工記不住哪個案例該回流到哪個 plan」的**索引問題**。被動 Detection + 人工 Capture，**不做 Enforcement / Promotion**。plan 各自宣告 evidence-rule + acceptance-gate（**notify 為 gate projection，不獨立**；notify = 可以 review ≠ 已升級）；memory 只存 pointer 不存 evidence。**Phase 0.5 schema freeze 拍板**：禁止 confidence 數字（用 `criteria_hits[]`）、candidate inbox **gitignored**（accept 才寫回 plan）、lifecycle `create → accept|discard|expire`（expire≠discard 防 accept-rate 失真）、scanner `agent_invoked`（hook 留作 promotion）、兩條 invariant **candidate 不可指向 candidate** + **scanner MUST be stateless**（防觀察層自我繁殖／長出第二條 authority）。**Phase 0.5 設計凍結完成**（design property only）；parse/建檔移到 Phase 1 Entry Check。**Phase 1 完成**：1A contract（schema+attach+criteria）→ 1B 人工 round-trip（證明 `artifact→candidate→plan evidence` 鏈 + lifecycle）→ 1C **assembler** scanner v0（Go `cmd/evidencecandidate`，Go-first；**非 dispatch target**、非 hook/runtime；只組裝人工標註 candidate，不 infer/match/rank/accept；3 硬護欄 removable / output=artifact / exit≠maturity；`criteria_hits` 須源於 scanner 外，matcher 留 Phase 2）。location 拍板 `governance/evidence-candidates/`（observation infra owned by governance）：`schema/` + `evidence-rules/` **pointer-only**（rule 定義留 plan，這裡只 index，防雙 source）+ `inbox/` gitignored。ownership 鏈 plan defines → governance indexes → candidate references → evidence enters plan。**不接入 runtime**；Phase 2 deferred until `phase2_gate`（count≥20 / reviewed_ratio≥80% / accepted_ratio>50% / age_p95<30d）。本 plan 自己是 N=1，不抽共用 primitive（走 governance-pattern N≥5 紀律）。 |
| [`archived/2026-06-09-1040-experience-validation-pipeline-evolution.md`](archived/2026-06-09-1040-experience-validation-pipeline-evolution.md) | in-progress | Experience Validation Pipeline Evolution：記錄 responsive render-context governance 後續 open questions；Phase 1–3 已落地 Browser Evidence Metadata / Capture Envelope、Validation Coverage Model watch-list、Responsive domain downgrade criteria；<CONSUMER_PROJECT> route/history pilot 增加 interaction-context evidence pressure；Phase 4 Evidence Envelope 初步 spike 決定暫不 promotion，typed Context Taxonomy 仍等待更多 scenario pressure，不新增 runtime gate。 |
| [`archived/2026-06-12-1600-projection-break-evidence-taxonomy-experience-runtime.md`](archived/2026-06-12-1600-projection-break-evidence-taxonomy-experience-runtime.md) | ✅ completed | Projection Break → Evidence Taxonomy → Experience Runtime：Phase 0–4 完成；glossary candidate terms 已註冊；<CONSUMER_PROJECT> player pilot 為 downstream consumer（專案 plan 不進 Ai-skill） |
| [`archived/2026-06-10-1718-software-delivery-governance-invariants.md`](archived/2026-06-10-1718-software-delivery-governance-invariants.md) | ✅ completed (auto-detected) | Software Delivery Governance Invariants：把 browser capability fallback、side-effect counting、deploy env drift、nested repo dirty state、interrupted deploy、browser smoke 等 incident feedback 提煉成 reusable validation-family invariants。Phase 0–5 已完成：activation/escalation = routing pressure；root-cause hypothesis = evidence acquisition discipline；configuration readback = State Visibility / Evidence Chain-backed workflow evidence shape；`ownership-boundary-validation` 拆成 `task-scope-validation` + `ownership-awareness`；authority side effects = artifact / validation evidence shape；新增 10 個 scenario-first validation YAML；workflow docs 已加入 evidence shapes；glossary 新增 `authority_coupled_side_effects`、`configuration_readback_validation`、`task_scope_validation`、`ownership_awareness` candidate terms。`runtime-capability-validation` 與 `operational-transaction-closure` 暫不進 glossary；`claim_validation` 保持 hypothesis；未新增 runtime gate。 |
| [`archived/2026-06-10-0908-user-journey-validation-integration.md`](archived/2026-06-10-0908-user-journey-validation-integration.md) | ✅ completed (auto-detected) | User Journey Validation Integration：Phase 0–5 與 <CONSUMER_PROJECT> pilot 已完成；BDD 擁有 Journey Specification，validation workflow 負責 Journey Execution；criticality selection criteria、`validation_scope`、`expected_outcomes` / `observable_evidence`、workflow gates、artifact gates、glossary terms 與 4 個 validation scenarios 已落地。Pilot 確認 API success 不足以證明 journey pass，`membership_active` 需 DB readback，`playback_allowed` 需 protected resource readback。 |
...
| [`archived/2026-05-22-1629-runtime-cognitive-modes-system.md`](archived/2026-05-22-1629-runtime-cognitive-modes-system.md) | ✅ completed | Runtime Cognitive Modes System：2026-09-09 讀回 route intentionally-manual；validators 按檔案路徑消費；grandfather 豁免於 2026-08-31 sunset。分類證據見 [sunset closeout](active/2026-07-08-0825-delegation-verification-arbitration-loop/evidence/2026-09-09-grandfather-sunset-closeout.md)；不等同 runtime-observed coverage。 |
...
| [`archived/2026-05-20-1501-cognitive-state-evidence-governance.md`](archived/2026-05-20-1501-cognitive-state-evidence-governance.md) | ✅ completed | Cognitive State & Evidence Governance：2026-09-09 讀回 route auto-detected；evidence hierarchy contract consumed；grandfather 豁免於 2026-08-31 sunset。分類證據見 [sunset closeout](active/2026-07-08-0825-delegation-verification-arbitration-loop/evidence/2026-09-09-grandfather-sunset-closeout.md)；不等同 runtime-observed coverage。 |
| [`archived/2026-05-20-1745-memory-retrieval-activation-governance.md`](archived/2026-05-20-1745-memory-retrieval-activation-governance.md) | ✅ completed | Memory Retrieval & Activation Governance：2026-09-09 讀回 route auto-detected；grandfather 豁免於 2026-08-31 sunset。分類證據見 [sunset closeout](active/2026-07-08-0825-delegation-verification-arbitration-loop/evidence/2026-09-09-grandfather-sunset-closeout.md)；不等同 runtime-observed coverage。 |
| [`archived/2026-05-20-1802-model-aware-execution-routing.md`](archived/2026-05-20-1802-model-aware-execution-routing.md) | ✅ completed | Model-Aware Execution Routing：2026-09-09 讀回 route auto-detected；grandfather 豁免於 2026-08-31 sunset。分類證據見 [sunset closeout](active/2026-07-08-0825-delegation-verification-arbitration-loop/evidence/2026-09-09-grandfather-sunset-closeout.md)；不等同 runtime-observed coverage。 |
