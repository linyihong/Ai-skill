...
| [`active/2026-07-08-0825-delegation-verification-arbitration-loop/_plan.md`](active/2026-07-08-0825-delegation-verification-arbitration-loop/_plan.md) | in-progress | Delegation Verification & Arbitration Loop：三角色委派閉環（orchestrator 規劃/切分/仲裁不執行、executor 憑 brief 執行、verifier fresh-context 只產證據）。baseline = 03-subplan-agent-delegation（completed）。doc-only 協議 + 雙 dogfood（software-delivery 外部 repo ×1 + Ai-skill ×1，**Cursor-first transport**，prompt kit 見 `01-dogfood-prompt-kit.md`）；不動 schema、不接 runtime、不建自動 orchestrator；schema promotion gated on 證據（deadline 2026-08-31）。 |
