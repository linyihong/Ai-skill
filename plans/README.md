...
| [`active/2026-07-14-0856-ui-pattern-knowledge-workflow/_plan.md`](active/2026-07-14-0856-ui-pattern-knowledge-workflow/_plan.md) | in-progress（research closed） | UI Knowledge：RC1+RC2 ✅ Closed；三層 Stable；維護期見 `maintenance-governance.md`；Method 🟡 Replicated once；**不規劃 RC3**。 |
| [`active/2026-07-08-0825-delegation-verification-arbitration-loop/_plan.md`](active/2026-07-08-0825-delegation-verification-arbitration-loop/_plan.md) | in-progress | Delegation Verification & Arbitration Loop：三角色委派閉環（orchestrator 規劃/切分/仲裁不執行、executor 憑 brief、verifier fresh-context 只產證據）。**Execute 意圖 = mandatory loop**（2j 裁決）；dogfood 2a–2s（含跨域 **2e Research** + **2s Architecture**；Knowledge 仍缺 → stage 2 = 2/3）→ 證據在 `evidence/`。研究線：ERA（Evidence constrains Decision，v2 凍結為工作模型、v3 候選）+ Phase 3 三分類決策框架，批次裁決 deadline 2026-08-31。doc-only、不動 schema、不建自動 orchestrator。 |
