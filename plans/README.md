...
| 檔案 | 狀態 | 說明 |
|------|------|------|
| [`active/2026-07-08-0825-delegation-verification-arbitration-loop.md`](active/2026-07-08-0825-delegation-verification-arbitration-loop.md) | draft | Delegation Verification & Arbitration Loop：三角色委派閉環（orchestrator 規劃/切分/仲裁不執行、executor 憑 brief 執行、verifier fresh-context 只產證據）。baseline = 03-subplan-agent-delegation（completed）。doc-only 協議 + 雙 dogfood（software-delivery 外部 repo ×1 + Ai-skill ×1）；不動 schema、不接 runtime、不建自動 orchestrator；schema promotion gated on 證據（deadline 2026-08-31）。 |
