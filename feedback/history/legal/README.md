...
| 分類 | 數量 | 說明 |
|------|------|------|
| [`common/`](common/) | 4 | 跨任務類型的通用 lesson（背調、策略推理、驗收與付款結構、風險分級、交付物權利鏈、已簽署合約的產出形狀、委外範本變更邊界與主機捆綁） |
...
| Slug | 摘要 |
|------|------|
| `common/2026-07-31_014200-change-boundary-and-bundled-hosting-in-dev-templates` | 供應方委外範本的變更五步／默示驗收應吸收；但「範圍內免加價」無可判定邊界、以及開發費捆綁主機／維運卻無 SLA，仍是 Stage 4 高頻缺口。結構齊全 ≠ 缺漏已清。 |
| `common/2026-07-31_064500-signed-contract-review-produces-different-artifacts` | 合約已簽署時「修正條文」無收件人。Stage 5 改為存續義務盤點＋風險分級的可行動事項＋下份合約改善清單；且事實狀態（履約／收款／停滯歸責）是 intake 輸入而非背景。 |
