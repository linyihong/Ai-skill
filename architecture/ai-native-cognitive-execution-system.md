...
| [ADR-013](../constitution/ADR-013-cognitive-role-primitive-gate.md) Cognitive Role Primitive Gate | Gen 3 子系統擴充 | **Completed** — Reject D1；Accept D2（`context.stance`）；Architecture Evolution Pattern closed 2026-07-07 |
...
### 12. Cognitive Modes（4 維 Mode Primitives）

每個任務依 discovery signals 解析四個維度，驅動對應子系統行為：

| 維度 | 值域 | 驅動子系統 |
|------|------|------------|
| `execution_mode` | FAST / NORMAL / DEEP / FORENSIC / RECOVERY | phase_machine allowed/forbidden_actions |
| `context_mode` | FULL / SUMMARY_FIRST / INDEX_ONLY / CHECKLIST_ONLY / GRAPH_ONLY | compression level（runtime alias） |
| `governance_mode` | LIGHT / STANDARD / STRICT / LOCKDOWN | blocking_gates gate_set_activation |
| `memory_mode` | NONE / EPISODIC / DECISION_REPLAY / FAILURE_REPLAY / PROJECT_CONTEXT | memory subdir + activation threshold AND-compose |

Runtime canonical：

- [`runtime/cognitive-modes.yaml`](../runtime/cognitive-modes.yaml) — 4 維 primitives + defaults + gate_activation
- [`runtime/cognitive-modes-discovery.yaml`](../runtime/cognitive-modes-discovery.yaml) — 14 discovery signals，從 raw signals 解析 mode
- [`runtime/cognitive-modes-phase-integration.yaml`](../runtime/cognitive-modes-phase-integration.yaml) — execution_mode → phase_machine floors
...
- `runtime.db` 表：`cognitive_modes`（mode resolution records）、`discovery_signals`（signal rules）
