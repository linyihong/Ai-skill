...
| `ai-skill kge check` | KGE push-watershed adapter（D9）：`RunAvailable` over staged/workspace Context → Validation fail=exit 30；Advisory 僅摘要（前 3 條）+ pointer，**不**因 advisory 失敗。邏輯在 `portable/kge`，本命令只組 Context + 呈現 | 否 | 是（讀 staged） | KGE Phase 4 |
