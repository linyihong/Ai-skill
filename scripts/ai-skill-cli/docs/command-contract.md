...
| `ai-skill hooks run commit-msg` | 執行 Git commit-msg hook logic：commit body 必須含 Cognitive Contract v2 報告，並通過 behavioral validators（execution floors、governance consistency、memory subdir、cognitive cost、activation signals、capability snippet、plan sync、token budget、adaptive triggers、bootstrap thinness、CLI doc sync、runtime YAML projection、Markdown/YAML sync）。Compact form 只適用於 all-default 且 staged diff 未命中 runtime / routing / workflow executable contract / active plan 等會要求 DEEP+STRICT 的 surfaces；compact path 也必須跑 staged-file validators。Validation 通過後附加 D9 `kge_advisory` **count-only** warning（不展開內文、不 block）。 | 否 | 是 | ADR-008 / runtime-cognitive-contract-v2 |
| `ai-skill hooks run post-commit` | 執行 Git post-commit hook logic（cursor bundle sync 等） | 否 | 是 | Shell To Go |
| `ai-skill hooks run pre-push` | 執行 Git pre-push hook logic：push-governance replay → **`kge check`（D9；validation 可 block、advisory 不擋）** → CLI source 變動時 go test ./... preflight | 否 | 是 | Shell To Go / KGE Phase 4 |
| `ai-skill hooks run session-start` | 執行 Claude Code SessionStart hook：query runtime.db、讀 4 個 bootstrap 文件、輸出 hookSpecificOutput JSON、寫 SessionStart flag（TTL 120s） | 是（`os.TempDir()/ai-skill-sessionstart-<hash>.flag`） | 否 | Cross-platform Go script runtime |
| `ai-skill hooks run pre-tool-use` | 執行 Claude Code / Cursor PreToolUse hook（host detect via payload `cursor_version`；plan 2026-06-05-0200 Phase 1-3）：scan transcript for Bootstrap Receipt + canonical read-log + workflow `primary_source` Read；Read-class tools allow（Claude `Read`；Cursor `read_file`/`ReadFile`/`functions.ReadFile`/`Glob`/`functions.Glob`/`rg`/`functions.rg`/`SemanticSearch`/`functions.SemanticSearch`/`list_dir`/`grep`/`glob_file_search`/`codebase_search`）；其他 tool 在 Receipt 或 workflow gate 未通過時 block via `exit 0` + host-native deny JSON（Claude `hookSpecificOutput.permissionDecision="deny"`；Cursor `{"permission":"deny",user_message,agent_message}`）—— **不再用 exit 30**（host 視為 non-blocking）。 | 是（`os.TempDir()/ai-skill-bootstrap-<hash>.done` + workflow side-channel） | 否 | Cross-platform Go script runtime |
...
| `ai-skill plans tree` | 純讀 `<root>/plans/active` + `archived` frontmatter，依 `parent` 指標建樹（`--state` / `--format text\|json\|markdown`）。observation companion，不 enforce | 否 | 否 | plan-tree-hierarchy-governance Phase 3 |
| `ai-skill plans validate` | **薄 CLI consumer**：load `<root>/plans/active\|archived`（reuse `scanAllPlanFrontmatter`，scope A）→ 呼叫 engine entrypoint `planvalidate.Validate` → 輸出 findings（`--format text\|json`）。CLI = transport only，零驗證邏輯；blocking finding → exit 30。非 goals：custom plans dir / schema dialect / external path（屬 Q8/Phase 3） | 否 | 否 | plans-system-portability Phase 2.4 |
| `ai-skill kge check` | KGE push-watershed adapter（D9）：`RunAvailable` over staged/workspace Context → Validation fail=exit 30；Advisory 僅摘要（前 3 條）+ pointer，**不**因 advisory 失敗。邏輯在 `portable/kge`，本命令只組 Context + 呈現。pre-push hook 委派同一呈現。 | 否 | 是（讀 staged） | KGE Phase 4 |
| `ai-skill kge validate [--advisory]` | KGE validation adapter；`--advisory` 輸出完整 advisory 列表（D9）。同樣委派 `portable/kge` | 否 | 是（讀 staged） | KGE Phase 4 |
| `ai-skill kge diagnose` | KGE IDE/MCP diagnostic adapter（D9）：stdout JSON array of `{severity,path,message,rule_id,code}`（完整 findings）；validation error 時 exit 30，advisory-only 仍 exit 0 | 否 | 是（讀 staged） | KGE Phase 4 |
| `ai-skill plans folderize` | 偵測 `plans/{active,archived}/` 頂層同 timestamp-slug 前綴的多檔 plan cluster，預覽或套用 folder 佈局（`_plan.md` + `NN-<suffix>.md`）。`--dry-run` / `--apply` 二選一；`--cluster <slug>` 可選。commit-msg `validatePlanTreeFolderConvention` 對 staged cluster 成員發 flat-cluster warning | 是（`--apply`） | 否 | plan-tree UI convention |
...
| `ai-skill hooks run commit-msg` validator `validateEnforcementRuleRegistrySync` | commit-msg hook 第 21 個 validator（Phase 5 Item 3，dual-gate companion to Phase 3 compile-time `orphan_rule`）：staged diff 含 `enforcement/*.yaml`（排除 `enforcement-registry.yaml` 本身與 `enforcement-registry.md`）且該檔案宣告 top-level `id:` 但未被 `enforcement-registry.yaml` source_files 綁定、且 `enforcement-registry.yaml` 也未同 commit staged，則 block。提早把 orphan 錯誤從下一次 `ai-skill runtime compile` 拉到 commit-msg；compile-time `orphan_rule` 仍為 canonical lint。opt-out `[skip-enforcement-registry-sync]` | 否 | 是 | mechanical-enforcement-registry Phase 5 |
...
> **Phase 5 receipt enrichment**（2026-06-02）：`ai-skill runtime receipt` 在原本 2 行 Bootstrap Receipt 之後追加第 3 行 `Enforcement: classes=<N> mechanical=<n> behavioral=<n> not_mech=<n> pending=<n> research=<n> deprecated=<n>`，由 `buildEnforcementCoverageSummaryLine` 重用 Phase 4 `buildCoverageReport` 計算，與 `ai-skill enforcement coverage` 永遠一致。registry 不可解析時降為 `skipped` check（不阻斷 receipt）。
