...
| 為此加的檢查本身空轉 | 檢查回報 OK 而從未比對任何東西，漂移照樣發生，且此時**沒有人會再去看** |
...
7. **新 gate 首次「通過」同樣先懷疑 gate 自己。** 這是第 6 條缺的另一半，而且更危險：**空轉的檢查會回報 OK**。它不像失敗清單那樣要求你去看，它只是安靜地讓你以為已經驗證過。常見成因是解析對象取錯範圍 —— 例如只掃宣告的 literal，而真正的值住在它引用的常數裡，於是迴圈跑了零次。
8. **任何新檢查在被採信前，對它宣稱涵蓋的每一個方向各做一次會紅的 mutation。** 通過不是證據，「會因為錯的東西而失敗」才是。同時確認失敗訊息**指名對象**：一個只說「不一致」而不說哪個 key 的檢查，在下一次真的紅掉時無法被行動。
...
- **它剛剛通過了 —— 我確認過它「有在比對東西」嗎？** 迴圈跑了幾次？解析出幾個對象？零個也會通過。
- 我對每個宣稱涵蓋的方向都做過一次會紅的 mutation 了嗎？失敗訊息說得出是哪個 key 嗎？

## 驗證

1. 存在一個會因不一致而失敗的機械檢查，且已接上 pre-commit／pre-push 或 CI
2. 該檢查對每一類不一致都經 mutation 驗證（各自製造一次，確認會紅且訊息指名對象）
3. 該檢查非空轉：可指出它實際解析到的對象數量，且該數量與受檢範圍相符
4. 檢查涵蓋雙向，且權威方在訊息中明示
5. 刻意允許的例外（單側獨有、機器面專用）在檢查中**具名記錄**並附理由，而非以靜默排除實現
