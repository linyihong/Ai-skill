...
**搜不到不等於不存在。** 「不存在」是一個需要證據的斷言，而搜尋失敗只是證據不足 —— 它同時可能是範圍不夠或用詞不對。
...
3. **在文件裡搜「這件事有沒有被記載」時，搜它描述的行為，不要搜你給它的識別碼。** 決策編號、ticket 代號、內部代稱通常不會出現在契約文件裡 —— 那些文件寫的是規則本身。實例：以 `grep D14` 斷定契約文件未載明某項授權例外，實則該節寫得完整（含旗標邊界與稽核意涵），只是不引用決策編號。這個誤判的方向特別糟：它會讓 agent 去補寫一份**已經存在**的說明，製造第二處敘述。
4. **找到既有實作時，先讀它的放置理由再決定移動它。** 註解裡的「為什麼在這裡而不在那裡」是設計資訊，不是雜訊。
5. **若既有理由與計畫衝突，停下來提出，而不是照計畫續建。** 兩者都可能對；決定權不在執行者手上。
6. **搬移既有實作時，區分「可達性」與「偏好」。** 若正式環境需要它（而它目前的位置在部署中不可達），移動有硬理由；若只是覺得該放別處，就不是。
...
- 找到了嗎？找到的話，它為什麼在那裡？
- **我搜的是識別碼還是行為？** 若在找「文件有沒有寫這件事」，識別碼（決策編號／ticket）通常不會在那份文件裡。

## 驗證

1. Preflight 紀錄中列出實際掃描的路徑範圍，且包含非產品目錄
2. 「不存在」的結論附帶搜尋詞與範圍，可被他人複驗
3. 若結論是「文件未記載」，所用搜尋詞為該事的行為敘述而非其識別碼
4. 若發現既有實作與計畫衝突，該衝突寫入證據文件並提交決策，而非由執行者逕行取捨
